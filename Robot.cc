...
void run_over_ramp(Create create, Create::ir_values ir, int rampCounter){
    ...
    fprintf(file, "Go over ramp activated");
    rampCounter = 0;
    while(true){
        ir = create.read_ir();
...
int main()
{
    ...
    int ii = 0;
    CvSVM svm;
    fprintf(file, "loading learned_lib");
    ...
    if(!capture)
    {
        fprintf(file, "Could not initialize capturing...");
        return -1;
    }
    ...
    cv::Mat dst;
    cvtColor(img_mat, dst, CV_RGB2GRAY);
    cv::resize(dst, dst, cv::Size(320,240), 0, 0, INTER_LINEAR);
    // perform SVM test
